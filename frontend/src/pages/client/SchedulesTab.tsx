...
function CronDisplay({ s }: { s: Schedule }) {
  return (
    <span className="font-mono text-xs text-slate-500">
      {s.cronMinute} {s.cronHour} {s.cronDayOfMonth} {s.cronMonth} {s.cronDayOfWeek}
    </span>
  )
}
...
function ScheduleCard({ schedule, serverId }: { schedule: Schedule; serverId: string }) {
  ...
  const [task, setTask] = useState<{ action: 'command' | 'power' | 'backup'; payload: string; timeOffset: number; sequence: number }>({ action: 'command', payload: '', timeOffset: 0, sequence: schedule.tasks.length + 1 })
  ...
  return (
    <div className="panel rounded-xl overflow-hidden">
      <div className="flex items-center gap-3 p-4">
        <button onClick={() => setOpen(v => !v)} className="text-slate-600 hover:text-slate-300 transition-colors">
          {open ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
        </button>
        <div className="flex-1 min-w-0">
          <p className="text-white font-medium text-sm">{schedule.name}</p>
          <CronDisplay s={schedule} />
        </div>
      ...
      {open && (
        <div className="border-t border-white/[0.06] px-4 py-3 space-y-2">
          ...
          {schedule.tasks.length === 0 && !showAddTask && (
            <p className="text-slate-600 text-xs py-2">No tasks. Add one to make this schedule do something.</p>
          )}
          ...
          {showAddTask && (
            <div className="pt-2 space-y-2 border-t border-white/[0.04]">
              <div className="grid grid-cols-3 gap-2">
                <select
                  value={task.action}
                  onChange={e => setTask(p => ({ ...p, action: e.target.value as any }))}
                  className={inputCls}
                >
                ...
                <input
                  value={task.payload}
                  onChange={e => setTask(p => ({ ...p, payload: e.target.value }))}
...
export default function SchedulesTab({ serverId }: { serverId: string }) {
  ...
  const [form, setForm] = useState({
    name: '', cronMinute: '*/5', cronHour: '*', cronDayOfMonth: '*', cronMonth: '*', cronDayOfWeek: '*',
  })
